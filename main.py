...
import configparser
import json
import logging # Librería por defecto que permite escribir archivos .log para registrar problemas en la base de datos
import os
import sqlite3
...
#
logging.basicConfig(
    filename="opdb-app.log", # ---> Todos los mensajes que produce la aplicación
...
def get_db():
    ...
    if "db" not in g:
        ...
        try:
            ...
            g.db.row_factory = sqlite3.Row # ---> Esto es un diccionario
...
def getparts():
    ...
    result = (
        get_db().execute("select part_no, quant from part_inventory_app").fetchall() #---> fetchall devuelve una lista de los registros de la base de datos como una lista de diccionarios
    )
...
def requestparts():
    ...
    part_no = request.form["part_requested"]  # ---> Estos nombres aparecen definidos en el index.html, en el ingreso de cadenas de texto en las líneas
    req_amt = request.form["amount_requested"] # ---> Estos nombres aparecen definidos en el index.html, en el ingreso de cadenas de texto en las líneas
